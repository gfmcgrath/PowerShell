...
Get-Mailbox -ResultSize Unlimited -RecipientTypeDetails UserMailbox | ForEach {Get-MailboxFolderPermission -Identity "$($_.PrimarySMTPAddress):\Calendar" -User Default} | Export-Csv mailboxescalendar.csv